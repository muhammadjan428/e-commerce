...
import { ChevronLeft, ChevronRight, Sparkles, Star, Search } from "lucide-react";
          ...
          <div className="px-3 sm:px-4 md:px-6 pb-6">
            ...
            <div className="mb-8 sm:mb-10 md:mb-12 text-center">
              <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-blue-100 to-purple-100 rounded-full text-blue-800 text-xs sm:text-sm font-medium mb-3 sm:mb-4">
                {searchQuery ? <Search className="w-3 h-3 sm:w-4 sm:h-4" /> : <Sparkles className="w-3 h-3 sm:w-4 sm:h-4" />}
                <span>{searchQuery ? 'Search Results' : 'Premium Collection'}</span>
              </div>
            ...
            {/* Pagination */}
            <div className="flex flex-col sm:flex-row justify-center items-center mt-12 sm:mt-16 gap-4 sm:gap-6">
              {/* Previous Button */}
              <div className="flex items-center gap-2 sm:gap-4">
                {page <= 1 ? (
                  <span className="flex items-center gap-1 sm:gap-2 px-4 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl text-gray-400 cursor-not-allowed bg-white/50 backdrop-blur-sm border border-gray-200 text-sm sm:text-base">
              ...
              <div className="flex gap-2 sm:gap-3">
                {page > 1 && (
                  <Link
                    href={buildUrl(page - 1, category)}
                    className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-xl sm:rounded-2xl text-gray-600 hover:text-blue-600 bg-white/70 backdrop-blur-sm hover:bg-white border border-gray-200 hover:border-blue-300 transition-all duration-300 transform hover:scale-110 shadow-lg text-sm sm:text-base"
                  >
                    {page - 1}
                  </Link>
                )}
                <span className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold shadow-lg shadow-blue-500/25 text-sm sm:text-base">
                  {page}
                </span>
                {hasNextPage && (
                  <Link
