...
import { getAllCustomers } from '@/lib/actions/customer.actions'; // your provided function
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      // Aggregate revenue/orders by date
      const dateMap = new Map<string, { revenue: number; orders: number }>();
      for (const customer of customers) {
        if (customer.lastOrderDate) {
          const date = format(new Date(customer.lastOrderDate), 'yyyy-MM-dd');
          const prev = dateMap.get(date) || { revenue: 0, orders: 0 };
          dateMap.set(date, {
            revenue: prev.revenue + customer.totalSpent,
            orders: prev.orders + customer.totalOrders,
          });
        }
      }
      ...
      return (
        <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm font-semibold text-gray-800">
            {metric === 'revenue' ? `Revenue: $${payload[0].value.toLocaleString()}` : `Orders: ${payload[0].value}`}
          </p>
        </div>
      );
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-white/70 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-sm font-medium text-gray-600 uppercase tracking-wide">Total Customers</h2>
          ...
          <Card className="bg-white/70 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-sm font-medium text-gray-600 uppercase tracking-wide">Total Revenue</h2>
